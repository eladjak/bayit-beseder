import type { Metadata, Viewport } from "next";
import { Heebo } from "next/font/google";
import { Toaster } from "sonner";
import { ThemeProvider, ThemeScript } from "@/components/ThemeProvider";
import { LanguageProvider } from "@/components/LanguageProvider";
import "./globals.css";

const heebo = Heebo({
  variable: "--font-heebo",
  subsets: ["hebrew", "latin"],
  // Reduced from 5 weights to 4 — 300 is rarely used; saves ~15KB
  weight: ["400", "500", "600", "700"],
  display: "swap",
  preload: true,
});

export const metadata: Metadata = {
  title: {
    default: "בית בסדר - ניהול תחזוקת הבית",
    template: "%s | בית בסדר",
  },
  description:
    "אפליקציה חינמית בעברית לניהול תחזוקת הבית לזוגות, משפחות ושותפים — תכנון שבועי חכם, גיימיפיקציה, רשימת קניות משותפת, Google Calendar ו-WhatsApp",
  keywords: [
    "ניהול בית",
    "ניהול משק בית",
    "אפליקציית ניהול משק בית",
    "תחזוקת הבית",
    "משימות בית",
    "חלוקת תורנויות",
    "חלוקת משימות בית",
    "זוגות",
    "משפחה",
    "שותפים לדירה",
    "ניהול בית לשותפים",
    "אפליקציה למשפחות עם ילדים",
    "תכנון שבועי",
    "רשימת קניות משותפת",
    "גיימיפיקציה",
    "אפליקציה עברית",
    "בית בסדר",
    "ניקיון הבית",
    "שלום בית",
    "ארגון הבית",
    "bayit beseder",
    "home management app",
    "household chores app",
    "shared home management",
  ],
  metadataBase: new URL("https://www.bayitbeseder.com"),
  alternates: {
    canonical: "https://www.bayitbeseder.com",
    languages: {
      "he-IL": "https://www.bayitbeseder.com",
      "en-US": "https://www.bayitbeseder.com",
    },
  },
  robots: {
    index: true,
    follow: true,
    googleBot: {
      index: true,
      follow: true,
      "max-image-preview": "large",
      "max-snippet": -1,
    },
  },
  verification: {
    google: "X9pkJAZuvtfG0-NomU_6uv9i47aG2_apJjFAzUFle9A",
  },
  manifest: "/manifest.json",
  appleWebApp: {
    capable: true,
    statusBarStyle: "default",
    title: "בית בסדר",
  },
  icons: {
    icon: [
      { url: "/favicon.svg", type: "image/svg+xml" },
      { url: "/icons/icon-192.png", sizes: "192x192", type: "image/png" },
    ],
    apple: "/icons/apple-touch-icon.png",
  },
  openGraph: {
    title: "בית בסדר — ניהול הבית ביחד, בכיף",
    description:
      "אפליקציה חינמית בעברית לניהול הבית לזוגות, משפחות ושותפים. משימות עם גיימיפיקציה, תכנון שבועי חכם, רשימת קניות משותפת ופרסים!",
    url: "https://www.bayitbeseder.com",
    siteName: "בית בסדר",
    images: [
      {
        url: "https://www.bayitbeseder.com/og-image.jpg",
        width: 1200,
        height: 630,
        alt: "בית בסדר — אפליקציה לניהול הבית לזוגות, משפחות ושותפים",
      },
    ],
    locale: "he_IL",
    type: "website",
  },
  twitter: {
    card: "summary_large_image",
    title: "בית בסדר — ניהול הבית ביחד, בכיף",
    description:
      "אפליקציה חינמית בעברית לניהול הבית לזוגות, משפחות ושותפים. משימות עם גיימיפיקציה, תכנון שבועי חכם, רשימת קניות משותפת ופרסים!",
    images: ["https://www.bayitbeseder.com/og-image.jpg"],
  },
};

export const viewport: Viewport = {
  themeColor: "#4F46E5",
  width: "device-width",
  initialScale: 1,
  maximumScale: 1,
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    // suppressHydrationWarning: ThemeScript runs in <head> BEFORE React hydration
    // and adds "light"/"dark" + --color-primary to <html>. Without this flag, every
    // page logs a hydration-mismatch warning to the console (the "1 Issue" badge).
    <html lang="he" dir="rtl" suppressHydrationWarning>
      <head>
        <ThemeScript />
        {/* Preconnect to Google Fonts to reduce font loading latency */}
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link
          rel="preconnect"
          href="https://fonts.gstatic.com"
          crossOrigin="anonymous"
        />
        {process.env.NODE_ENV === "production" && (
          <script
            defer
            src="https://plausible.io/js/pa-NZ0Fcei1zpOAEbuBVEy5g.js"
          />
        )}
      </head>
      <body className={`${heebo.variable} font-sans antialiased`}>
        <ThemeProvider>
          <LanguageProvider>
          {children}
          </LanguageProvider>
          <Toaster
            position="top-center"
            dir="rtl"
            toastOptions={{
              className: "font-sans",
            }}
          />
        </ThemeProvider>
      </body>
    </html>
  );
}
