...
## 2026-09-25 — Meal planner v1 built (branch `feat/meal-planner-v1`, PR open, NOT merged)

**Status: full v1 built per the intake-free "start from sensible defaults" instruction — 50 new tests, tsc clean, `next build` clean, `/meals` page verified in a local screenshot. Migration 015 NOT run against production (forbidden by task scope) — that's Elad's tap.**

- **Data (`supabase/migrations/015_meals.sql`, still unapplied):** extended the existing `meals`/`meal_plan` tables (idempotent `ADD COLUMN IF NOT EXISTS`/`DROP+ADD CONSTRAINT`, safe to re-run) — added `meals.ingredients jsonb`, made `meal_plan.meal_id` nullable, added `leftovers` to the status enum.
- **Defaults:** `src/lib/meals/seed-data.ts` — 25 Hebrew family dinners (kashrut-tagged meat/dairy/parve, never mixed — enforced by `hasConflictingKashrutTags` + a test). `src/lib/meals/seed.ts` seeds them per-household on first `/api/meals` or `/api/agent/prep` call, only if the household has zero meals — never as global rows.
- **Generator (`src/lib/meals/generator.ts`):** pure, deterministic (no RNG). Rules: no repeat within `max(5, meal.min_repeat_days)` days (5-day floor even if the meal's own field is lower), optional ≤2-meat-in-a-row, Friday prefers a `shabbat`-tagged meal, Saturday reuses an earlier `leftovers-friendly` meal as `status:"leftovers"`, busy days prefer `light`-tagged meals.
- **Defrost reminder (`src/lib/meals/defrost.ts` + `src/lib/meals/timezone.ts`):** "מה להפשיר הערב" — DST-correct Asia/Jerusalem math (own offset calculator, verified against both a December and an August date; 19:00 IL = 17:00 UTC in winter, 16:00 UTC in summer). Exposed via `GET /api/agent/prep?householdId=<uuid>` (was a 501 stub, now real — Bearer `BAYIT_AGENT_KEY`), returns JSON + a ready Hebrew WhatsApp line.
- **Fast actions:** `PATCH /api/meals/plan/day` — swap (`GET` returns 3 alternatives via `src/lib/meals/swap.ts`), cooked/skipped, leftovers (`src/lib/meals/leftover-shift.ts` — shifts the rest of the week forward a day, stopping at the first already-cooked day), per-day note.
- **Shopping:** `GET/POST /api/meals/shopping-suggest` — suggests this week's ingredients (`src/lib/meals/shopping.ts`), de-duplicated against the open shopping list (normalized title match) and merged across meals (same ingredient in 2 meals = 1 line with combined quantity). User confirms before anything is inserted into the existing, unmodified `shopping_items` table.
- **UI:** `/meals` page (new bottom-nav tab) — week view, today's card with defrost banner + swap/cooked/leftovers/skip buttons, add-your-own-meal form, shopping-suggestion sheet. Mobile-first, RTL, matches the app's existing component/Tailwind conventions.
- **Tests:** 50 new (generator 7, defrost 8, shopping 5, swap 4, leftover-shift 4, timezone 6, seed+seed-pack-integrity 5, `/api/meals/plan` 3, `/api/meals/plan/day` 4, `/api/agent/prep` 4). Full suite: **41 files / 392 tests, all green.** `tsc --noEmit` exit 0. `next build` succeeds, `/meals` listed as a static route. Red→green proven by hand for the repeat-window floor, the 36h defrost window, and the shopping dedupe (each: broke it, watched the right test fail for the right reason, restored, watched it pass).
- **What's still Elad's tap:** run migration 015 in Supabase (branch first, per this repo's own safe-live-refactor convention) · merge the PR · optionally wire a nightly 21:00 push of `/api/agent/prep`'s `whatsappText` through Kami (the endpoint is ready; no cron was added — out of this task's scope).
